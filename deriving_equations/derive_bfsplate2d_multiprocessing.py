...
def integrate(ind_integrand):
    ind, integrand = ind_integrand
    print(ind)
    return sympy.integrate(integrand, ('xi', -1, 1), ('eta', -1, 1))
...
def main():
    ...
    u = sympy.Matrix(sympy.var(','.join([('c%02d' % i) for i in range(4*DOF)])))
    em = Bm*u
    eb = Bb*u
    Nm = A*em + B*eb
    Nxx = Nm[0]
    ...
    #NOTE integration of Kge is very slow...
    print('Integrating Kge')
    ind_integrands = []
    for ind, integrand in np.ndenumerate(Kge):
        ind_integrands.append([ind, integrand])
    p = Pool(cpu_count)
    a = list(p.map(integrate, ind_integrands))
