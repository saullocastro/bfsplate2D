...
from bfsplate2d import BFSPlate2D, update_K, update_M
...
rho = lam.rho
D = 2*h**3*E/(3*(1 - nu**2))
wmn = (m**2/a**2 + n**2/b**2)*np.sqrt(D*np.pi**4/(2*rho*h))
print('Theoretical omega123', wmn)
print(omegan[:num_eigenvalues])

from structsolve import freq
from panels import Shell
s = Shell(a=a, b=b, rho=rho, laminaprop=(E, nu), stack=[0], plyt=h)
freq(s.calc_kC(), s.calc_kM())
